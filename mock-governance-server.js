...
function checkGovernance(request) {
    ...
    const isAllowed = Array.from(ALLOWED_MODELS).some((allowed) => modelBase.includes(allowed) || allowed.includes(modelBase));
    if (!isAllowed) {
        const response = {
            approved: false,
...
server.listen(PORT, "127.0.0.1", () => {
    ...
    console.log(`[MockAPI] Allowed models: ${Array.from(ALLOWED_MODELS).join(", ")}`);
});

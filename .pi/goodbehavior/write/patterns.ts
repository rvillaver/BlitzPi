...
export const KEYS = ["emDash", "negParallel", "aphorism", "ruleOfThree", "inflatedAbs", "reversal"] as const;
export type PatternKey = (typeof KEYS)[number];
export type Counts = Record<PatternKey, number> & { words: number };
...
export const per1k = (c: Counts, k: PatternKey): number => (c[k] / c.words) * 1000;

if (import.meta.main) {
  const mode = process.argv[2];
  const sets = JSON.parse(process.argv[3]) as Record<string, string[]>;  // {label: [paths]}
  ...
  for (const k of KEYS) {
    ...
    console.log(k.padEnd(14) + labels.map((_l, i) => (v[i].toFixed(1) + " (" + raw[i] + ")").padEnd(13)).join("")

...
Count them with `.blitz/goodbehavior/write/patterns.ts` (per 1000 words). **Counts are a yardstick, not a verdict.**
...
```sh
bun .blitz/goodbehavior/write/fingerprint.ts --diff <before-file-or-dir> <after-file-or-dir>
bun .blitz/goodbehavior/write/patterns.ts "label" '{"before":["<path>"],"after":["<path>"]}'
```

    ...
    if (token) {
      const discord: DiscordAdapter = new DiscordAdapter({
        ...
        onSlash: async ({ sub: cmd, options, conv, user, guildOwnerId }) => {
          ...
          if (cmd === "threads") { await bridge.op("settings", { conv: key, threads: options.mode }); return `Threads: **${options.mode}** — ${options.mode === "on" ? "activity and answers in the shared thread" : options.mode === "answer" ? "activity in the shared thread; answers follow where you ask (channel or thread)" : "everything here"}.`; }

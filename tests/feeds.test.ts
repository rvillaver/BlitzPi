...
describe("feeds hook on bash tool calls", () => {
  ...
  test("layer, summary line and config default", () => {
    const cfg: any = { threat_detection: { enabled: true, tier: 2 }, audit: { enabled: true, path: "/a" }, profiles: { default: "user" }, sandbox: { enabled: true, run_dir: ".", backend: "auto" }, governance: { enabled: true, mode: "enforce", provider: "local" }, goodbehavior: { profile: "development" }, threat_api: { enabled: false }, feeds: { packages: "monitor", secrets: "monitor", cache_ttl_hours: 24 } };
    ...
    expect(loadConfig().feeds).toEqual({ packages: "enforce", secrets: "monitor", cache_ttl_hours: 24 });

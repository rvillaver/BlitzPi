...
describe("security status model", () => {
  test("one vocabulary: per-call governance is monitor, gates are enforce, pinned bash is monitor", () => {
    ...
    expect(L).toEqual({ input: "enforce", governance: "enforce", profiles: "enforce", sandbox: "enforce", bash: "enforce", threat: "enforce", feeds: "enforce", secrets: "off", commands: "off", urls: "off", audit: "enforce" });
  ...
  test("summary line names provider, backend, profile, tier with modes", () => {
    expect(summaryLine(cfg, "bwrap")).toBe("governance local (enforce) · profile user (enforce) · files (enforce) · bash bwrap (enforce) · threat tier 2 (enforce) · packages osv (enforce) · secrets gitleaks (off) · commands sigma (off) · urls urlhaus (off) · audit (enforce)");
  });

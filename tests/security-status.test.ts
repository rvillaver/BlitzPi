...
describe("security status model", () => {
  test("one vocabulary: per-call governance is monitor, gates are enforce, pinned bash is monitor", () => {
    ...
    expect(L).toEqual({ input: "enforce", governance: "enforce", profiles: "enforce", sandbox: "enforce", bash: "enforce", threat: "enforce", content: "monitor", feeds: "enforce", secrets: "off", commands: "off", urls: "off", audit: "enforce" });
